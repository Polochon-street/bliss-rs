...
crate instead of ffmpeg to handle the decoding of songs. See the
[Decoders](#decoders) section below for more information.

However, it makes bliss-rs easier to package, since it removes a big dependency to ffmpeg.
...
To use Symphonia instead of FFmpeg to decode songs, use the symphonia-all feature flag instead:
        ...
        cargo run --release --example=playlist --no-default-features --features=serde,aubio-static,symphonia,symphonia-all /path/to/folder /path/to/first/song
